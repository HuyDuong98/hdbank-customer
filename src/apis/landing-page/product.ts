...
export const getListProductByFilter = async (
  langCode: any,
  page: any,
  pageSize: any,
  filterId: any
...
export const getProductDetail = async (
  ...
  const apiUrl = `${
    DOMAIN.hdbankBetaV1
  }/LandingPage/landingpage-product-detail?productId=${id}&langCode=${
    langCode || "vi"
  }`;
  return getAsync<IProductDetailDto>(apiUrl);
};

export const getListCompare = async (
  langCode: string,
  lstCardID: any[]
): Promise<any> => {
  const getParam =
...
export const getListOtherProduct = async (
  langCode: string,
  lstCardID: any[]
): Promise<any> => {
  const getParam =
...
export const getSelectItemCustomerJourney = async (
  langCode: string
): Promise<any> => {
