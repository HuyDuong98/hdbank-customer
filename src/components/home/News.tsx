...
const News: FC<INewsProps> = (props) => {
  ...
  const rederHighLightNews = () => {
    const image = !isMobile ? highlight?.banner_pc_mode : highlight?.banner_mobile_mode
    return (
            ...
            <Grid container direction="column">
              <Grid container alignItems="center" className={Style.dateWrap}>
                <div className={Style.arrow}>
                  <div className={Style.innerArrow}>{highlight?.categoryName}</div>
                </div>
                <Typography variant="body2">{highlight?.published_date}</Typography>
              </Grid>
              <Typography
                variant="body1"
  ...
  return (
    <Grid container justifyContent="center" className={Style.newsWrap}>
      <Grid container justifyContent="center" className={Style.container}>
        <Grid container justifyContent="center" alignItems="center" className={Style.headingWrap}>
        ...
        <Grid container spacing={isXsSize ? 0 : 5} className={`${Style.otherNewsWrap} ${Style.noneMg}`}>
          {list?.map((news, idx) => {
            const { title, content, published_date, banner_mobile_mode, banner_pc_mode, id, categoryName } = news
            return (
                  ...
                  <Grid item sm={12} xs={6}>
                    <Grid container direction="column">
                      <Grid container alignItems="center" className={Style.dateWrap}>
                        ...
                        <Typography variant="body2">{published_date}</Typography>
                      </Grid>
                      <Typography
                        variant="body1"
      ...
      <Grid className={Style.btnDiv}>
        ...
        >
          <Link href={pagePath.news}><a>{t('seeAll')}</a></Link>
        </Button>
      </Grid>
