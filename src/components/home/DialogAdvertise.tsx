...
import { Box, Button, Dialog, Divider, IconButton, Typography } from '@material-ui/core'
...
const DialogAdvertise: FC = () => {
  ...
  const { data } = useQuery(['advertiseQuery', langCode], () => getAdvertise(langCode))
        ...
        >
          <Box className={Style.dialogContent}>
            <Link href={data.btn_url || '#'}>
              <a>
                <img src={isMobile ? data.mobile_mode : data.pc_mode} alt={data.title} title={data.title} />
              </a>
            </Link>
            ...
            <Box className={Style.action}>
              <Button variant="contained" color="primary" onClick={() => (window.location.href = data.btn_url)}>
                {data.btn_text || t('Đăng ký ngay')}
              </Button>
            </Box>
          </Box>
          <IconButton size="small" className={Style.close} onClick={handleClose}>
            <CloseRoundedIcon />
          </IconButton>
        </Dialog>
      )}
