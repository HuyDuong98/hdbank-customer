...
import { getListCompare, getListOtherProduct } from '../../apis/landing-page/product'
...
const lstBreadCrumb = [{ label: 'productList', path: pagePath.productPage }, { label: 'compareCards' }]
...
const CompareProducts: FC = () => {
  ...
  const { data, isFetching } = useQuery(['compareQuery', langCode, listQuery], () =>
  ...
  const renderEmptyBox = () => {
    return (
      <Box className={Style.colDetail}>
        <Grid container direction="column" alignItems="center" className={Style.itemProduct}>
    ...
    return (
      <Container>
        <Grid className={Style.pageWrap}>
          ...
          <Box mt={3} mb={3}>
            <Typography variant="h1">{t('compareCards')}</Typography>
          </Box>
        </Grid>
                ...
                return (
                  <Box key={i} className={`${Style.colDetail} ${Style.first}`}>
                    <Grid container direction="column" alignItems="center" className={Style.itemProduct}>
                      ...
                      <Link href={`${pagePath.productPage}/${convertUrltoSlug(title)}.${id}`}>
                        <Typography className={Style.linkPC}>{t('seeDetail')}</Typography>
                      </Link>
